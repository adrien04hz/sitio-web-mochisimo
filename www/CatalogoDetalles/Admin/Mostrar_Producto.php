...
try {
    ...
    $producto = $_POST['producto_categoria']; // Convertir a entero para seguridad
    ...
    $queryDescripcion = "SELECT Prod.id AS producto_id, Cat.nombre AS categoria, Cat.imagen_categ AS categoria_imagen, Prod.precio, Prod.nombre AS producto, 
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){  
           ...
           $base64Imagen1 = base64_encode($row['categoria_imagen']);
            $respuesta1 .= "<div class='col-4' style='text-align: center; margin-top: 10px;'>";
            $respuesta1 .= "<strong>Imagen de Categoria</strong> <br><img src='data:image/jpeg;base64," . $base64Imagen1 . "' alt='Imagen del producto' style='max-width: 100%; height: auro; border-radius: 5px;'>";
            $respuesta1 .= "</div>";
            
            
            $respuesta1 .= "<div style='with:60px; text-align:left;'><strong>Producto:</strong> <br>" . htmlspecialchars($row['producto']) . "</div>";
            $respuesta1 .= "<div style='with:140px; text-align:justify;'><h4>Descripción:</h4> <br> " . htmlspecialchars($row['descripcion']) . "</div>";
            $respuesta1 .= "<div style='with:100px; text-align:justify;'><strong>Frase:</strong> <br>" . htmlspecialchars($row['frase']) . "</div>";
           ...
           // Convertir la imagen binaria a base64 para poder mostrarla
            ...
            $respuesta1 .= "<div class='col-4' style='text-align: center; margin-top: 10px;'>";
            $respuesta1 .= "<strong>Imagen de Producto</strong> <br><img src='data:image/jpeg;base64," . $base64Imagen . "' alt='Imagen del producto' style='max-width: 100%; height: auto; border-radius: 5px;'>";
            $respuesta1 .= "</div>";
